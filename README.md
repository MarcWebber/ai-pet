...
文本输入不会调用 Accessibility 整段赋值、Playwright `fill` 或剪贴板粘贴。执行器按字符
输入，保留标点和换行停顿；较长内容会偶尔出现一次打错后立即退格纠正，最终输入值仍与
Agent 给出的完整文本一致。编程题的少量注释由 Agent 根据代码含义生成，输入器不会自行
改写答案。
...
当前 `main` 包含一组从 `52fc081` 开始演进的未发布接管重构，不能把开发分支直接视为
